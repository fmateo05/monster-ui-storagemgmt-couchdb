define(function(require) {
	...
	var storageManager = {
		...
		storageManagerRender: function(pArgs) {
			...
			self.getStorage(function(data) {
				...
				for (var i = 0, len = storagesData.length; i < len; i++) {
					storagesData[i].logo = self.storages['couchdb'].getLogo();
				}
		...
		storageManagerFormatData: function(data) {
			...
			if(data && data.hasOwnProperty('connections') && Object.keys(data.connections).length > 0) {
...
//     						var storageData = data.connections.connection;
		...
		storageManagerShowNewItemPanel: function(){
			...
			for (var i = 0, len = storagesList.length; i < len; i++) {
				...
				data.push({
					name: keyword,
					type: keyword,
					logo: self.storages['couchdb'].getLogo(),
		...
		storageManagerMakeConfig (storageKeyword, data, uuid) {
			...
			storageData = {
                        ...
                        //storageData.connections[uuid] = uuid;              
